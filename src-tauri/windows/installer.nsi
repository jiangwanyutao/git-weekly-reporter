; 本文件复制自 Tauri 官方 NSIS 安装器模板（tauri-cli v2.10.0：
;   crates/tauri-bundler/src/bundle/windows/nsis/installer.nsi），只做了以下改动：
;   1. 新增 SHORTCUTNAME，桌面 / 开始菜单快捷方式和「应用」列表里的显示名用它（中文「AI周报」）
;   2. 安装或自动更新时，把老版本以产品名命名的快捷方式改成新名字
; 产品名 PRODUCTNAME 保持英文不变：安装记录的注册表键、自动更新都依赖它。
; 升级 @tauri-apps/cli 后，需要把新版官方模板重新复制过来，再按上面两点改一遍。
...
; 老版本快捷方式以产品名命名：确认指向本程序后改成 SHORTCUTNAME；新名字已存在就删掉旧的
!macro MigrateShortcutName DIR
  !insertmacro IsShortcutTarget "${DIR}\${PRODUCTNAME}.lnk" "$INSTDIR\${MAINBINARYNAME}.exe"
  Pop $0
  ${If} $0 = 1
    ${If} ${FileExists} "${DIR}\${SHORTCUTNAME}.lnk"
      Delete "${DIR}\${PRODUCTNAME}.lnk"
    ${Else}
      Rename "${DIR}\${PRODUCTNAME}.lnk" "${DIR}\${SHORTCUTNAME}.lnk"
    ${EndIf}
  ${EndIf}
!macroend
...
Function CreateOrUpdateStartMenuShortcut
  ...
  !insertmacro MigrateShortcutName "$SMPROGRAMS\$AppStartMenuFolder"
  !insertmacro MigrateShortcutName "$SMPROGRAMS"
...
Function CreateOrUpdateDesktopShortcut
  ; We used to use product name as MAINBINARYNAME
  ; migrate old shortcuts to target the new MAINBINARYNAME
  !insertmacro MigrateShortcutName "$DESKTOP"
