...
{{commits}}`,
  },
  {
    id: 'few-commits',
    name: '提交少的一周（逐条写）',
    description: '本周只有几条提交时用，每个功能点单独成条，不被合并成一条',
    content: `你是一位资深的技术负责人，要把一周的开发记录写成一份**给团队和上级看**的周报。请始终记住：读者不写代码，也不知道任何内部代号。

**本模板适用于提交较少的一周。** 提交少不代表工作少，你的任务是把每一件做成的事都讲清楚，而不是把它们压缩成一句话。

**输入说明：**
下面的提交记录已经按「项目 → 模块」预先归好组。
- 同一模块下内容相同、只是反复提交的记录，属于同一件事，合并成一条。
- 预处理附带的「体量小」「其中 N 个体量够独立成条」这类标注，是给提交多的周报准备的，**本模板一律忽略**，不要据此合并或删减条目。
...
**3. 每个有意义的功能点单独成条**
- 做成了一件事就写一条：新增的能力、修复的问题、改进的体验、完成的配置或上线，各自单独成条。
- **只有说的是同一件事**（同一个功能的多次提交、同一个问题的反复修改）才合并成一条。
- 不同的功能点**不要**为了显得精炼而合并，也**不要**塞进「其他优化与修复」。
- 本周条目本来就少，每一条都要写充实：说清楚改了什么、解决了谁的什么问题、带来了什么效果，两到三句为宜。
- 只有全部都是版本号、格式调整这类零碎记录时，才用一条「其他优化与修复」简短带过；没有这类记录就不要写这一条。

**4. 只写有价值的成果**
忽略版本号变更、合并提交、格式调整、临时提交这类没有信息量的记录，不要为了凑条数把它们单独写成一条。
若某组标注了「另有 N 条同类改动未逐条列出」，说明这块工作量更大，请在描述里体现其分量。

**5. 输出前必须自检一遍**
- 数一数：输入里有几个不同的功能点，输出就应该有几条，少了就是合并过度，回头拆开。
- 凡是出现带等号或括号的代码写法、带单位的数值参数、驼峰或下划线拼写的英文标识符、全大写常量、只有本项目开发者才看得懂的英文缩写，一律改写成人话。
确认没有问题了，再给出最终结果。
...
>
> **【项目A名称】**
> 1. **订单导出能力上线：** 用户可按时间范围导出明细，替代了原先手工整理表格的流程，财务对账时间明显缩短。
> 2. **登录异常问题修复：** 修复了网络波动时登录后被立即退出的问题，报错提示改为用户能看懂的说明。
>
> **【项目B名称】**
> 1. **权限校验统一：** ……

**下方列出了几个项目，就必须写几段，一个都不能漏。**

**以下是本周的提交记录（已按项目 / 模块归组，请逐个功能点写清楚）：**
{{commits}}`,
  },
  {
    id: 'summary',
